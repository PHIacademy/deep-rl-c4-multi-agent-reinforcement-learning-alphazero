...
This document walks through `alphazero-TicTacToe.ipynb`, `ConnectN.py`, `Play.py`, and `MCTS.py` — covering how to set up, play, train, and test an AlphaZero agent, as well as how the game engine, rendering, and Monte Carlo Tree Search (MCTS) are implemented under the hood.
...
- **Part 1 — Using the Notebook**: follows `alphazero-TicTacToe.ipynb` top to bottom — setting up a game, playing it, building the neural network, training an agent, and testing it. Treats the MCTS search as a "black box" that picks good moves.
- **Part 2 — Under the Hood**: opens up `ConnectN.py`, `Play.py`, and especially `MCTS.py` to explain exactly how that black box is implemented — useful if you want to modify, extend, or debug the code.
...
| File | Purpose |
|---|---|
| `ConnectN.py` | The **game environment**. A general "Connect-N in a row" game (Tic-Tac-Toe is just a 3×3, N=3 special case). Tracks board state, current player, and score. |
| `MCTS.py` | The **Monte Carlo Tree Search** engine used by AlphaZero. Builds a search tree, explores it using a neural network's guidance, and picks moves. |
| `Play.py` | An **interactive/visual game runner** built on `matplotlib`. Lets a human click moves, or watches AI vs AI games animate. |
| `alphazero-TicTacToe.ipynb` | The **notebook** that ties everything together: defining the neural network ("Policy"), training loop, and playing against the trained agent. |
...
This part opens up `ConnectN.py`, `Play.py`, and `MCTS.py` to explain how the notebook-level black boxes from Part 1 are actually implemented — useful if you want to modify, extend, or debug them.

<a id="generalized-connect-n"></a>
## 8. `ConnectN.py` Internals
...
## 10. `MCTS.py` Internals — Deep Dive
